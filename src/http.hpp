...
consteval Response make_response(const char (&s)[N]) { return {s, N - 1}; }
...
inline constexpr std::size_t kBufSize = 2048;
...
public:
    ...
    [[noreturn]] void run() {
        ...
        for (;;) {
            ...
            for (int i = 0; i < n; ++i) {
                auto fd = events_[unsigned(i)].data.fd;
                auto rev = events_[unsigned(i)].events;

                if (fd == listen_fd_) { accept_all(); continue; }

                auto ui = unsigned(fd);
                if (RINHA_UNLIKELY(ui >= conns_len_ || !conns_[ui].in_use)) continue;

                bool alive = !(rev & (EPOLLERR | EPOLLHUP));
                if (alive && (rev & EPOLLIN))    alive = drain(conns_[ui]);
                if (alive && (rev & EPOLLRDHUP)) alive = false;
...
private:
    ...
    int create_listener() const {
        return mode_ == Mode::kUnix ? listen_unix() : listen_tcp();
    }

    int listen_tcp() const {
    ...
    void accept_all() {
        for (;;) {
            ...
            auto ui = unsigned(cfd);
            if (RINHA_UNLIKELY(ui >= kMaxConns)) { ::close(cfd); continue; }
            if (ui >= conns_len_) conns_len_ = ui + 1;

            auto& c = conns_[ui];
            c.fd = cfd; c.have = 0; c.in_use = true;

            epoll_event ev{.events = EPOLLIN | EPOLLRDHUP | EPOLLET, .data = {.fd = cfd}};
            if (::epoll_ctl(ep_fd_, EPOLL_CTL_ADD, cfd, &ev) != 0) {
                c.in_use = false; ::close(cfd);
            }
        }
    }
    ...
    void close_conn(int fd) {
        ...
        ::close(fd);
        auto ui = unsigned(fd);
        if (ui < conns_len_) { conns_[ui].in_use = false; conns_[ui].fd = -1; }
    }

    bool drain(Connection& conn) {
        for (;;) {
            if (RINHA_UNLIKELY(conn.have == kBufSize)) return false;
            auto r = ::read(conn.fd, conn.buf + conn.have, kBufSize - conn.have);
            if (RINHA_LIKELY(r > 0)) {
                conn.have += uint32_t(r);
    ...
    [[gnu::always_inline]]
    static const char* find_hdr_end(const char* d, std::size_t n) {
        return static_cast<const char*>(::memmem(d, n, "\r\n\r\n", 4));
    }
    ...
    static int parse_content_length(const char* hdr, std::size_t len) {
        auto* p = static_cast<const char*>(::memmem(hdr, len, "ength:", 6));
    ...
    static std::string_view extract_path(const char* req, std::size_t len) {
        auto* sp1 = static_cast<const char*>(std::memchr(req, ' ', len));
        if (RINHA_UNLIKELY(!sp1)) return {};
        ++sp1;
        auto rem = len - std::size_t(sp1 - req);
        auto* sp2 = static_cast<const char*>(std::memchr(sp1, ' ', rem));
        if (RINHA_UNLIKELY(!sp2)) return {};
        return {sp1, std::size_t(sp2 - sp1)};
    }
    ...
    bool process(Connection& conn) {
        ...
        while (consumed < avail) {
            ...
            const char* hdr_end = find_hdr_end(cur, rem);
            if (!hdr_end) break;

            auto hdr_len = std::size_t(hdr_end - cur) + 4;
            int clen = parse_content_length(cur, hdr_len);
            auto total = hdr_len + std::size_t(clen);
            if (rem < total) break;

            auto path = extract_path(cur, hdr_len);
            auto body = std::string_view(hdr_end + 4, std::size_t(clen));
            if (!send_all(conn.fd, dispatch(path, body))) return false;
            consumed += total;
        }

        if (consumed > 0) {
            auto remaining = uint32_t(avail - consumed);
            if (remaining > 0) std::memmove(conn.buf, conn.buf + consumed, remaining);
            conn.have = remaining;
        }
    ...
    static bool send_all(int fd, Response resp) {
        std::size_t off = 0;
        while (off < resp.len) {
            auto n = ::send(fd, resp.data + off, resp.len - off, MSG_NOSIGNAL);
            if (RINHA_LIKELY(n > 0)) { off += std::size_t(n); continue; }
            if (n < 0 && errno == EINTR) continue;
            if (n < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
                pollfd pfd{fd, POLLOUT, 0};
                int r; do { r = ::poll(&pfd, 1, 2); } while (r < 0 && errno == EINTR);
    ...
    struct ConnStore { Connection c[kMaxConns]{}; };
    ConnStore* store_ = new ConnStore();
    Connection* conns_ = store_->c;
